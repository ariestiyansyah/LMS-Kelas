...
from braces.views import LoginRequiredMixin, PermissionRequiredMixin, JsonRequestResponseMixin, CsrfExemptMixin
    ...
    def post(self, request):
        for id, order in self.request_json.items():
            Module.objects.filter(id=id,
                    course__xman=request.user).update(order=order)
    ...
    def post(self, request):
        for id, order in self.request_json.items():
            Content.object.filter(id=id),
                            module__course__xman=request.user) \
                            .update(order=order)
        return self.render_json_response({'saved': 'OK'})